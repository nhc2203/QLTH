...
export default QLNH = function ({ navigation }) {
  //Animated
  ...
  const getList = () => {
    axios
      .get(
        'https://opensheet.elk.sh/1CU0JOnN3HZfViWBoOCqurpp537vJ90Gt_MPd9fm-lxg/HoaDonNhap',
      )
  ...
  const handleSave = async () => {
    if (hideId == null) {
      axios
        ...
        .catch(error => {
          Alert.alert("Thong Bao", "Loi:" + error);
        })
      console.log("Co*****", flag);
      if (flag == true)
        Alert.alert("Thong Bao", "Thanh Cong");
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity onPress={() =>
        navigation.navigate("ChiTietNhapHang", {
          MAHD: item.MAHD,
        })
      }>
        <Animated.View
          ...
          <View style={styles.wrapText}>
            <Text style={styles.fontSize}>{index + 1}.{' '}{item.TENHD}</Text>
    ...
    <SafeAreaView>
      ...
      <View style={{ height: pxWindow.height * 0.18, borderColor: 'red', borderWidth: 0 }}>
        <View style={styles.header_container}>
          ...
          <Text style={styles.txt_main}>Nhập Hàng</Text>
        ...
        <Modal animationType="slide" visible={modalVisible} style={styles.form}>
          <ScrollView>
            <View style={styles.form}>
              ...
              <Text style={{fontSize: 16, fontWeight: 'bold'}}>Tên hóa đơn lập:</Text>
              <TextInput
                value={tenHd}
              ...
              />
              <Text style={{fontSize: 16, fontWeight: 'bold'}}>Nhà Cung Cấp:</Text>
              <Dropdown
                ...
                labelField="label"
                valueField="value"
                placeholder={!isFocus ? 'Mã Nhà Cung Cấp' : '...'}
              ...
              <View>
                <TouchableOpacity onPress={() => {
                  ...
                  if (tenHd == null) {
                    Alert.alert('Thông báo', 'Vui lòng nhập tên hóa đơn');
                    return;
                  }
                  handleSave()
                }}
